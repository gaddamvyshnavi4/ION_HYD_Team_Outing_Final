...
import { MemoriesPopupComponent } from './components/memories-popup/memories-popup.component';
import { SharedStylesComponent } from './components/shared-styles/shared-styles.component';

@Component({
  selector: 'app-root',
  standalone: true,
  imports: [RouterOutlet,FormsModule,CommonModule , HttpClientModule, ImageCarouselComponent, BuzzerComponent, WelcomeComponent, WelcomePopupComponent, MemoriesPopupComponent, SharedStylesComponent],
...
teams : any[] = [];
  numberOfTeams = 4;
  teamNames: string[] = ['ఉత్తరం', 'దక్షిణం', 'తూర్పు', 'పడమర'];
 ...
 if (this.refreshSub) {
  ...
  updateTeamCount(): void {
    const defaultNames = ['ఉత్తరం', 'దక్షిణం', 'తూర్పు', 'పడమర'];
    this.teamNames = Array(this.numberOfTeams).fill('').map((_, i) => 
      this.teamNames[i] || defaultNames[i] || `Team ${String.fromCharCode(65 + i)}`
    );
  }
  ...
  async getTranslatedName(name: string): Promise<string> {
    if (!this.showTranslation) return name;
    
    try {
      const teluguTranslation = await this.translateText(name, 'te');
      const hindiTranslation = await this.translateText(name, 'hi');
      return `${teluguTranslation} / ${hindiTranslation}`;
    } catch (error) {
  ...
  getTranslatedNameSync(name: string): string {
    if (!this.showTranslation) return name;
    
    if (this.translationCache[name]) {
      return this.translationCache[name];
    }
    
    this.getTranslatedName(name).then(translated => {
      this.translationCache[name] = translated;
    });
    
    return name; // Return original name while translation loads
  }
}
